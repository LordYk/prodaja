...
module.exports = async (client, guild) => {
    ...
    /* Если не нашли по ID — ищем по имени среди существующих каналов */
    if (category === undefined) {
        category = guild.channels.cache.find(
            ch => ch.name === 'Rinfo' && ch.type === ChannelType.GuildCategory
        );

        if (category !== undefined) {
            /* Нашли существующую категорию — сохраняем её ID */
    ...
    if (category === undefined) {
        category = await DiscordTools.addCategory(guild.id, 'Rinfo');
        instance.channelId.category = category.id;
        client.setInstance(guild.id, instance);
    }
